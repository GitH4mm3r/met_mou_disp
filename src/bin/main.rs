...
fn main() {
    App::new()
        .add_plugins((DefaultPlugins.set(WindowPlugin {
            ...
            ..default()
        }),LogDiagnosticsPlugin::default(),FrameTimeDiagnosticsPlugin))
        // ClearColor must have 0 alpha, otherwise some color will bleed through
        .insert_resource(ClearColor(Color::NONE))
        .insert_resource(MouseTrail{delta_sum_trail:([(0,0);TRAIL_SIZE]),buffer_head:0,last_moved:0u32})
...
fn setup(mut commands: Commands, asset_server: Res<AssetServer>,mut windows: Query<&mut Window>) {
    ...
    commands.spawn((SpriteBundle {
        ...
        transform: Transform::from_xyz(0., 0., 0.).with_scale(Vec3::new(0.20,0.20,0.20)),
        ..default()
        }, 
    ));
    //Toggle passthrough
...
fn spawn_dots(mut commands: Commands, asset_server: Res<AssetServer>) { 
...
fn move_dots(mut m_trail:ResMut<MouseTrail>, mut dots: Query<(&mut DotI, &mut Transform, &mut Sprite)>) {
    ...
    if m_trail.last_moved == 0 {
        ...
        for (dot, mut transform, mut sprite) in &mut dots {
            ...
            let x_transposed = (m_trail.delta_sum_trail[dot.index as usize].0/DELTA_DIV)  as f32;
            let y_transposed = -(m_trail.delta_sum_trail[dot.index as usize].1/DELTA_DIV) as f32; 
            ...
            let size_scale:f32 = (trail_size  as f32 -0.75*(distance_from_head as f32))/(trail_size as f32);  
            ...
            if let Some(event) = manager.get_event() {
                match event {
                    ...
                    RawEvent::MouseMoveEvent(_,_,_) => {
                        if let RawEvent::MouseMoveEvent(_id,dx,dy) = event {
...
fn read_stream(receiver: Res<StreamReceiver>, mut events: EventWriter<StreamEvent>) {
    for (_per_frame,from_stream) in receiver.try_iter().enumerate() {
        events.send(StreamEvent(from_stream));
    }
}

fn read_stream_event(mut reader: EventReader<StreamEvent>, mut m_trail: ResMut<MouseTrail>) {
   
    for (_per_frame, event) in reader.read().enumerate() {
        let head = m_trail.buffer_head;
        if head > 0 { 
